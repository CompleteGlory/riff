...
Unit tests for `SuggestedUsersRepo` — fetches suggested accounts to follow, shown during new-user
onboarding. (It used to match phone contacts too; that went with the phone-OTP feature, since
matching depended on verified numbers — see `CLAUDE.md` → Known Bugs Fixed.) There's no cubit for this feature (the screen calls the repo
...
- **`Dio`** — mocked with `@GenerateMocks([Dio])`. This repo calls `Dio` directly rather than
